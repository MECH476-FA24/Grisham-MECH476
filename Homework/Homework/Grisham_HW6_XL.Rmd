...
data_filtered <- data_raw %>%
  filter(state == "CO") %>%
  select(text) %>%
  mutate(hashtags = str_extract_all(text, "#(\\d|\\w)+")) #XL: mutate is not essential here, just str_extract_all(text, "#(\\d|\\w)+"), then use nrow()
...
#XL: you can use your "data_wildfire_time" subdata set, and group_by(user) to summarize the number of tweets each senator published easily
