...
library(dplyr)
library(tibble)
data <- read_csv("ftc_o3.csv")
ozone_data <- data %>%
  select(sample_measurement, datetime) %>%
  drop_na() %>%
  rename(ozone_ppm_hourly = sample_measurement)
...
nrow_after <- nrow(ozone_data)
obs_drop <- nrow_before - nrow_after
print(paste("The number of observations dropped are", obs_drop),quote = FALSE)
...
df_max <- ozone_data %>%
  slice_max(order_by = ozone_ppm_hourly, n = 10)
print(df_max)
```
...
df_min <- ozone_data %>%
  slice_min(order_by = ozone_ppm_hourly, n = 10)
print(df_min)
```
...
print(paste("The highest point of the day occurs at", high_day, "and the lowest point of the day occurs at", df_min))
